...
static inline uint8_t get_sequence_counter(switch_pro_state_t* sw_pro) {
    uint8_t count = sw_pro->sequence_counter;
    sw_pro->sequence_counter = ((sw_pro->sequence_counter + 1) & 0x0F);
    return count;
}

static void switch_set_home_led(switch_pro_state_t* sw_pro, uint8_t daddr, uint8_t itf_num) {
    memset(sw_pro->buf_out, 0, sizeof(sw_pro->buf_out));
    ...
    report_out->commands[count++] = 0x0F; // always on
    report_out->commands[count++] = 0xF0; // max intensity
    report_out->commands[count++] = 0xF0; // max mini cycle
    tuh_hxx_send_report(daddr, itf_num, (const uint8_t*)report_out, 
                        sizeof(switch_pro_report_out_t) + count);
}

static void switch_init_2(switch_pro_state_t* sw_pro, uint8_t daddr, uint8_t itf_num) {
    memset(sw_pro->buf_out, 0, sizeof(sw_pro->buf_out));
    switch_pro_report_out_t* report_out = (switch_pro_report_out_t*)sw_pro->buf_out;
    ...
    case SW_PRO_2_SET_HOME_LED:
        switch_set_home_led(sw_pro, daddr, itf_num);
...
static void switch_init_1(switch_pro_state_t* sw_pro, uint8_t daddr, uint8_t itf_num) {
    switch_pro_command_t* cmd = (switch_pro_command_t*)sw_pro->buf_out;
    memset(sw_pro->buf_out, 0, sizeof(sw_pro->buf_out));
    cmd->report_id = SWITCH_PRO_REPORT_ID_HID;

    get_sequence_counter(sw_pro);

    switch (sw_pro->init_state) {
    case SW_PRO_1_HANDSHAKE:
        cmd->command = SWITCH_PRO_COMMAND_HANDSHAKE;
        tuh_hxx_send_report(daddr, itf_num, (const uint8_t*)cmd, 
                            sizeof(switch_pro_command_t));
        sw_pro->init_state = SW_PRO_1_UNK1;
        break;
    case SW_PRO_1_UNK1:
        cmd->command = 0x03;
        tuh_hxx_send_report(daddr, itf_num, (const uint8_t*)cmd, 
    ...
    case SW_PRO_1_DISABLE_TIMEOUT:
        ...
        tuh_hxx_send_report(daddr, itf_num, (const uint8_t*)cmd, 
                            sizeof(switch_pro_command_t));
        sw_pro->init_state = SW_PRO_1_UNK2;
        break;
    case SW_PRO_1_UNK2:
        cmd->command = 0x04;
        tuh_hxx_send_report(daddr, itf_num, (const uint8_t*)cmd, 
                            sizeof(switch_pro_command_t));
        sw_pro->init_state = SW_PRO_2_SET_PLAYER_LED;
        break;
    default:
        switch_init_2(sw_pro, daddr, itf_num);
        break;
    }
...
static void switch_pro_mounted(usbh_type_t type, uint8_t index, uint8_t daddr, uint8_t itf_num, 
    ...
    switch_init_1(sw_pro, daddr, itf_num);
}
...
static void switch_pro_report_received(uint8_t index, usbh_periph_t subtype, uint8_t daddr, 
    ...
    if (report_in->buttons[0] & SWITCH_PRO_BUTTON0_Y)   { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_x); }
    if (report_in->buttons[0] & SWITCH_PRO_BUTTON0_A)   { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_b); }
    if (report_in->buttons[0] & SWITCH_PRO_BUTTON0_B)   { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_a); }
    if (report_in->buttons[0] & SWITCH_PRO_BUTTON0_X)   { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_y); }
    if (report_in->buttons[0] & SWITCH_PRO_BUTTON0_R)   { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_rb); }
    if (report_in->buttons[0] & SWITCH_PRO_BUTTON0_ZR)  { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_rt); }

    if (report_in->buttons[1] & SWITCH_PRO_BUTTON1_MINUS)   { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_back); }
    if (report_in->buttons[1] & SWITCH_PRO_BUTTON1_PLUS)    { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_start); }
    if (report_in->buttons[1] & SWITCH_PRO_BUTTON1_L3)      { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_l3); }
    if (report_in->buttons[1] & SWITCH_PRO_BUTTON1_R3)      { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_r3); }
    if (report_in->buttons[1] & SWITCH_PRO_BUTTON1_HOME)    { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_sys); }
    if (report_in->buttons[1] & SWITCH_PRO_BUTTON1_CAPTURE) { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_misc); }

    if (report_in->buttons[2] & SWITCH_PRO_BUTTON2_DOWN)    { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_down); }
    if (report_in->buttons[2] & SWITCH_PRO_BUTTON2_UP)      { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_up); }
    if (report_in->buttons[2] & SWITCH_PRO_BUTTON2_RIGHT)   { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_right); }
    if (report_in->buttons[2] & SWITCH_PRO_BUTTON2_LEFT)    { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_left); }
    if (report_in->buttons[2] & SWITCH_PRO_BUTTON2_L)       { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_lb); }
    if (report_in->buttons[2] & SWITCH_PRO_BUTTON2_ZL)      { gp_report->buttons |= GP_BIT(sw_pro->profile.btn_lt); }

    gp_report->trigger_l = ((gp_report->buttons & GAMEPAD_BUTTON_LT) ? 0xFFU : 0x00U);
    gp_report->trigger_r = ((gp_report->buttons & GAMEPAD_BUTTON_RT) ? 0xFFU : 0x00U);
    ...
    if (memcmp(gp_report, &sw_pro->prev_gp_report, sizeof(gamepad_pad_t)) == 0) {
        tuh_hxx_receive_report(daddr, itf_num);
        return;
    }
    ...
    if (report_in->buttons[1] & SWITCH_PRO_BUTTON1_HOME) {
        switch_set_home_led(sw_pro, daddr, itf_num);
    }
    tuh_hxx_receive_report(daddr, itf_num);
}
